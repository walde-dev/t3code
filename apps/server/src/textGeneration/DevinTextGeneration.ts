...
export const makeDevinTextGeneration = Effect.fn("makeDevinTextGeneration")(function* (
  ...
  const runDevinJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      const resolvedModel = resolveDevinModelId(modelSelection.model);
      ...
      const runtime = yield* makeDevinAcpRuntime({
        ...
        // Text generation is background work: ambient credentials only, never
        // an interactive `devin-browser` login.
        browserAuth: false,
      }).pipe(
